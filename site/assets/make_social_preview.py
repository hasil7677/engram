"""Generates the GitHub social preview image (1280x640) for Engram.
...
# four-node motif on the right, echoing the four backing stores
nodes = [(900, 130), (1120, 110), (960, 260), (1160, 300), (1030, 400)]
edges = [(0, 1), (0, 2), (1, 3), (2, 3), (2, 4), (3, 4)]
for a, b in edges:
    draw.line([nodes[a], nodes[b]], fill=BORDER, width=2)
for i, (x, y) in enumerate(nodes):
    r = 9 if i in (0, 3) else 6
    fill = ACCENT if i in (0, 3) else PANEL
    outline = ACCENT_LIGHT if i in (0, 3) else BORDER
    draw.ellipse([x - r, y - r, x + r, y + r], fill=fill, outline=outline, width=2)

PAD = 90

# badge pill
badge_text = "CLOUD-HOSTED  \u00b7  MULTI-TENANT  \u00b7  MIT LICENSED"
...
# title
title_y = 150
draw.text((PAD, title_y), "Engram", font=f_title, fill=TEXT)

# tagline
tag_y = title_y + 130
tagline = "Give your chatbot memory across sessions."
draw.text((PAD, tag_y), tagline, font=f_tag, fill=MUTED)

# stat card, bottom left
stat_y = 430
label_y = stat_y + 90
draw.text((PAD, stat_y), "1,986", font=f_stat_num, fill=ACCENT2)
draw.text((PAD + 4, label_y), "LOCOMO QUESTIONS SCORED",
...
sbbox = draw.textbbox((0, 0), "10/10", font=f_stat_num2)
s_offset = (f_stat_num.getbbox("1,986")[3] - sbbox[3])  # baseline-align with the big number
draw.text((div_x + 40, stat_y + s_offset), "10/10", font=f_stat_num2, fill=ACCENT_LIGHT)
draw.text((div_x + 44, label_y), "CONVERSATIONS, FULL DATASET",
          font=f_stat_label, fill=MUTED)

# repo url, bottom right
url = "github.com/hasil7677/engram"
